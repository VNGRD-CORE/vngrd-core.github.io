...
function renderLoop(timestamp) {
    ...
    if (APP.lowerThird.visible) {
        ...
        const LT_COLORS = {
            guest: '#00f3ff', track: '#00ff88', breaking: '#ff3333',
            neon: '#ff00cc', split: '#ffaa00', scan: '#00ffaa'
        };
        const accent = LT_COLORS[ltPreset] || '#00f3ff';

        ctx.save();
        ctx.globalAlpha = 1;
        ctx.filter = 'none';

        // Scale-aware layout (matches CSS: bottom:8%, left:20px)
        const bx      = 20;
        const barH    = Math.round(h * 0.065);
        const barY    = Math.round(h * 0.92) - barH;
        const titleSz = Math.round(h * 0.022);
        const subSz   = Math.round(h * 0.012);

        // Measure text to size the background plate
        ctx.font = `bold ${titleSz}px Orbitron, Arial, sans-serif`;
        const titleW = ctx.measureText(ltTitle).width;
        ctx.font = `${subSz}px "JetBrains Mono", monospace`;
        const subW   = ctx.measureText(ltSub.toUpperCase()).width;
        const barW   = Math.max(titleW, subW) + 60; // padding + 3px accent strip

        // Background plate
        const grad = ctx.createLinearGradient(bx, barY, bx + barW, barY);
        ...
        ctx.fillRect(bx, barY, 3, barH);

        // Title
        ctx.font = `bold ${titleSz}px Orbitron, Arial, sans-serif`;
        ctx.fillStyle = '#ffffff';
        ctx.fillText(ltTitle, bx + 17, barY + Math.round(barH * 0.47));

        // Subtitle
        ctx.font = `${subSz}px "JetBrains Mono", monospace`;
        ctx.fillStyle = accent;
        ctx.fillText(ltSub.toUpperCase(), bx + 17, barY + Math.round(barH * 0.82));
...
function showLowerThird(preset) {
    ...
    // Anti-double: Remove .visible, force a reflow so the browser resets the
    // CSS transition, then re-apply. Rapid re-triggers (clicking a button while
    // an LT is already animating) will now always replay the slide-in cleanly.
    lt.classList.remove('visible');
    void lt.offsetWidth; // FORCE REFLOW — DO NOT REMOVE

    container.classList.remove('lt-guest', 'lt-track', 'lt-breaking', 'lt-neon', 'lt-split', 'lt-scan');
    ...
    // Cache text for canvas bake-in (renderLoop reads this to draw LT onto canvas)
    APP.lowerThird.title = $('lt-title-text').textContent;
    APP.lowerThird.subtitle = $('lt-subtitle-text').textContent;

    // Update button states
    ['guest', 'track', 'breaking', 'off'].forEach(p => {
        const btn = $(`btn-lt-${p}`);
        if (btn) btn.classList.toggle('on', p === preset);
    });
...
function ensureAudioChain() {
    ...
    try {
        ...
        APP.audio.outputLimiter.ratio.setValueAtTime(20, APP.audio.ctx.currentTime);
